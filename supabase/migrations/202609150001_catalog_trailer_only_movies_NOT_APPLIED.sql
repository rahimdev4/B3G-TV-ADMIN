-- Catalog V2 only: allow a published movie to have a ready trailer while its
-- full movie is not yet available. This replaces only cms_publish_title.
-- It never reads from or writes to the live legacy shows/videos tables.
-- Apply once in the Supabase SQL editor after reviewing this file.
...
begin
  ...
  if v_title.content_type = 'movie'
     and v_title.video_asset_id is null
     and v_title.trailer_asset_id is null then
    raise exception 'Movie requires a trailer or main video';
  end if;
  if exists (
...
grant execute on function public.cms_publish_title(uuid) to authenticated;
